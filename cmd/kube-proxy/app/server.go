...
func AddFlags(options *Options, fs *pflag.FlagSet) {
	...
	fs.Int32Var(options.config.IPTables.RejectBit, "iptables-reject-bit", utilpointer.Int32PtrDerefOr(options.config.IPTables.RejectBit, 13), "If using the pure iptables proxy, need to reject packets for service without endpoints.  Must be within the range [0, 31].")
